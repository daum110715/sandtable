...
**Sandtable**（产品暂定名：历史模拟推演AI）让用户从一个历史场景出发，自由改写历史事件，由 AI 实时推演后续走向：选择场景 → 在对话中自由输入任意改写 → 演员 Agent 根据当前世界状态推演结果 → 记录员 Agent 把状态变更写回结构化数据库 → 继续输入，循环往复。

### 为什么做这个项目？

> "如果当时采取另一种行动，接下来可能发生什么？"

这是人类对历史最本能的好奇。Sandtable 用结构化的世界状态数据库与不可变的事件日志，保证 AI 推演可追溯、世界状态一致--让用户探索"如果当时……接下来会怎样"，而非生成一个"看起来像历史"的故事。
...
用户在对话中输入任意颗粒度的历史改写（细到"那天江上刮西北风"），由 AI 根据当前世界状态推演后果。不是选择题，也不是结构化指令。
...
**必须跑通**：选择场景 → 自由输入改写 → 演员 Agent 推演 → 记录员 Agent 写回世界状态数据库 → 事件日志回溯 → 上帝面板展示，且世界状态不与前序矛盾。

**1.0 不包含**：上帝面板细节展示、历史范围锁定、权限分层/迷雾/代价机制、确定性重放/规则裁决引擎/裁判 Agent、显式世界线分支、账户与多人分享、商业化与部署。
