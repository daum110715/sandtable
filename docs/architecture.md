...
- D007/D014/D015/D016：Agent 框架、模型路由、Agent 接入方式、数据存储技术均为待定决策。
...
提供预设场景与自定义背景的加载，产出初始世界状态。它不预测推演结果。

**Interface**：按场景标识加载初始世界状态；接收自定义背景并构建初始世界状态。
...
把演员 Agent 的推演结果拆解为结构化状态变更，写回世界状态数据库，并产出事件记录。世界状态的写入集中在此。

**Interface**：输入推演结果与当前世界状态，输出结构化状态变更与事件记录。

### 裁判 Agent（可选/后期）

检查推演前后矛盾。1.0 默认不启用；状态一致性在 1.0 主要由记录员 Agent 的结构化写回与事件日志保证。
...
- Agent 框架（D007）、模型路由（D014）、Agent 接入方式（D015）、数据存储技术（D016）。
- 裁判 Agent 接入时机。
- 上帝面板具体展示内容（D029）。
