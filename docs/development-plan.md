...
已完成（M1+M2）：世界状态与事件 schema、Agent 输入输出协议、内存世界状态数据库、内存事件日志、推演编排（含幂等与逻辑事务）、赤壁场景与内存桩 Agent。

尚未开始：持久化（M3）、真实模型推演（M4）、对话 + 上帝面板前端闭环（M5）、1.0 硬化与发布闸门（M6）、模型凭证、部署。

待定决策：D007 Agent 框架、D014 模型路由、D015 Agent 接入方式、D016 数据存储技术。
