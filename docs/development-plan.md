...
已完成（M1+M2+M3）：世界状态与事件 schema、Agent 协议、内存与 SQLite 世界状态/事件日志、推演编排（幂等 + 同事务）、赤壁场景与内存桩 Agent、API 健康/就绪与最小 deduce 路径。

尚未开始：真实模型推演（M4）、对话 + 上帝面板前端闭环（M5）、1.0 硬化与发布闸门（M6）、模型凭证、部署。

待定决策：D007 Agent 框架、D014 模型路由、D015 Agent 接入方式。D016 已确认（SQLite，ADR-0007）。
